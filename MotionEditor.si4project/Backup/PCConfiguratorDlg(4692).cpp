...
void CPCConfiguratorDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDV_MinMaxFloat(pDX, m_fParamR, 0.005f, 40.0f);
	DDX_Text(pDX, IDC_EDIT_PARAM_SD, m_fParamSd);
	DDV_MinMaxFloat(pDX, m_fParamSd, 0.005f, 70.0f);
	DDX_Text(pDX, IDC_EDIT_PARAM_W, m_fParamW);
	DDV_MinMaxFloat(pDX, m_fParamW, 0.005f, 70.0f);
	DDX_Text(pDX, IDC_EDIT_PARAM_SPEED, m_fParamRs);
	DDV_MinMaxFloat(pDX, m_fParamRs, 50.0f, 1000.0f);
	DDX_Text(pDX, IDC_EDIT_CONV_ROLLING_SPEED, m_fParamCBs);
	DDV_MinMaxFloat(pDX, m_fParamCBs, 10.0f, 1255.0f);
...
BOOL CPCConfiguratorDlg::OnInitDialog()
{
	...
	CString strBuf;
	strBuf.Format(_T("Application Verions %su\n"), APP_VERION);//(LPCTSTR)(APP_VERION) );
  	m_log.LogWrite(strBuf);
	strBuf.Empty();
	strBuf.Format(_T("Log start: %4d/%2d/%2d %2d:%2d:%2d\n"), 
		tm.GetYear(),
		tm.GetMonth(),
		tm.GetDay(),
		tm.GetHour(),
...
//	int nAsked = 5;
//	int nAgree = 4;
//	strBuf.FormatMessage(_T("%1!d! of %2!d! developers agree: Hockey is %3%!"),
...
//void CPCConfiguratorDlg::OnGalilConnect()
//{
//	// TODO: Add your control notification handler code here
//	CTime tm;
//	tm = CTime::GetCurrentTime();//Get system time
//	char buf[1024];
//	memset(buf, 0, sizeof(buf));
//	sprintf_s(buf, "Log start: %4d/%2d/%2d %2d:%2d:%2d", 
//		         tm.GetYear(), 
//				 tm.GetMonth(), 
//				 tm.GetDay(),
//				 tm.GetHour(),
...
//				 tm.GetSecond());
//	m_log.LogStart();
////	m_log.LogWrite(buf); // TODO: not working, why?
//
//	// Get the Set IP
//	CString str;
//	//m_setIP.GetWindowTextW(str);
//	m_galil.Connect((GCStringIn)str.GetBuffer(0));
//	m_log.LogWrite((const char *)str.GetBuffer(0));
//}

//void CPCConfiguratorDlg::OnChangeInputLx()
//{
	...
	//CEdit* editHelp = ((CEdit*)(GetDlgItem(IDC_INPUT_LX)));
	...
	//{
	//	AfxMessageBox("³¬¹ý·¶Î§£¡");
	//	GetDlgItem(IDC_EDIT1)->SetWindowText(_T("100"));
	//}
...
{
	//m_editCtrlFont.CreatePointFont(100, _T("Arial"));
	//m_ecParamL.SetFont(&m_editCtrlFont);
...
BOOL CPCConfiguratorDlg::PreTranslateMessage(MSG* pMsg)
{
	...
	if ( (WM_KEYDOWN == pMsg->message && VK_RETURN == pMsg->wParam) ||
		 (WM_KILLFOCUS == pMsg->message)) // TODO: not working
	{
		...
		for (UINT i = 0; i < CTRLID_LIST_SIZE; i++)
		{
			...
			if (pFocusedWnd == GetDlgItem(uCtrlId))
			{
				if (!UpdateData(TRUE))
				{
					...
					//pFocusedWnd->SetWindowText((LPCTSTR)buf);
...
//		{ IDC_BTN_SET_UP,                   "BV", 3, CONTROL_ID_INVALID, "Set up" },
		...
		{ IDC_BTN_AXIS_Y_MINUS,             "XQ#Bmove2",        0, CONTROL_ID_INVALID, "Axis Y-" },
		{ IDC_BTN_AXIS_Y_PLUS,              "XQ#Bmove1",        0, CONTROL_ID_INVALID, "Axis Y+" },
		...
		{ IDC_BTN_CONVEYOR_WAIT_TIME_CHANGE,"Tw=",              0, CONTROL_ID_INVALID, "Change waiting time" },		
...
TCTRLPARAM CPCConfiguratorDlg::GetCtrlParameterInfo(UINT uCtrlID)
{
	TCTRLPARAM sParamList[] = {
		//											default, step,   min(na),    max(na),     	multiple
		{ "L",     IDC_EDIT_PARAM_L,              	0.005f, 0.005f, 0.005f, 160.0f,     2000, &m_fParamL },
		...
		{ "R",     IDC_EDIT_PARAM_R,              	0.005f, 0.005f, 0.005f, 80.0f,      2000, &m_fParamR },  // 2R < L
...
int CPCConfiguratorDlg::SendCommands(UINT uCtrlID)
{
	...
	else
	{
		...
		for (UINT i = 0; i < uCmdNum; i++)
		{
			...
			strBuf.Format(_T("%s%d"), sCmdList[i].strCmdPrefix, optionsValue);
			CT2A dest(strBuf.GetString());
			int cmdRet = m_galil.Command(dest);

			//memset(buf, 0, sizeof(buf));
			strBuf.Empty();
			//sprintf_s(buf, "%s: %s%d, ret:%d", sCmdList[i].strCmdDescrip, sCmdList[i].strCmdPrefix, optionsValue, cmdRet);
	        strBuf.Format(_T("%s: %s%d, ret:%d"), sCmdList[i].strCmdDescrip, sCmdList[i].strCmdPrefix, optionsValue, cmdRet);
			m_log.LogWrite(strBuf);
		}
	}
...
void CPCConfiguratorDlg::OnClickedBtnConnect()
{
	...
	else
	{
        ...
        CString strBuf;
        strBuf.Format(_T("Connection status: %d"), ret);
		m_log.LogWrite(strBuf);
	}
}
